...
def generated_text(query):
    ...
    prompt = '''{} create a Project Report Status based on the decription provided'''.format(query)
...
def main():
    st.title("Generate a  Project Report Status based")
    query = st.text_input("Enter the name of the topic:")
    
    if st.button("Generate"):
        presentation_text = generated_text(query)
        st.write(" ")
        st.write("Generate  Project Report Status based: ")
